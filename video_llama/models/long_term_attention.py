...
class LongTermAttention(nn.Module):
    def __init__(self, head_size:int , length: int, target_len:int,  attn_func: str, attn_num_basis: int,
                  ...
                  n_heads: int, affines: bool, mask: bool, mask_type: str, kl_regularizer: bool, sigma_0, mu_0, sticky_memories, sigmas, tau, **kwargs):
    ...
    def get_basis(self, length, target_len):
        ...
        if self.continuous:
            ...
            sigmas = self.sigmas # basis function sigmas
    ...
    def forward(self, k, q, new_doc, layer_n):
        ...
        if self.continuous:
            ...
            if new_doc:
                self.B_past=None 
                self.x_past=None
            k = k.reshape(batch_size, k.shape[-1], klen) # [B,e,L]    
            ...
            keys = B.view(batch_size,self.attn_num_basis,self.n_head,self.d_head).transpose(1,2) # [B,h,N,d]
            values = B.view(batch_size,self.attn_num_basis,self.n_head,self.d_head).transpose(1,2) # [B,h,N,d]
            ...
            if self.affines:
                sigma_sq = torch.var(scores, dim=-1, unbiased=False)
                mu = torch.mean(scores, dim=-1)
                mu = mu.view(-1)
            ...
            if output_density:
                ...
                if type(self.transform) == ContinuousSoftmax:
                    ...
                    density = ((1. / torch.sqrt(2*math.pi*sgm**2)) * torch.exp(
                        -((positions.unsqueeze(0) - mu.unsqueeze(1)) ** 2 /(2 * sgm ** 2)))).unsqueeze(1)
